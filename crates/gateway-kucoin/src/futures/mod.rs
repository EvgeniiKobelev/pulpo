...
pub struct KucoinFutures {
    ...
    rest: rest::KucoinFuturesRest,
}

impl KucoinFutures {
    pub fn new(config: ExchangeConfig) -> Self {
        let rest = rest::KucoinFuturesRest::new(&config);
        Self { config, rest }
    }
...
#[async_trait]
impl FuturesExchange for KucoinFutures {
    ...
    async fn liquidations(&self, _symbol: &Symbol, _limit: u16) -> Result<Vec<Liquidation>> {
        // KuCoin Futures does not expose public liquidation data via REST.
        Ok(vec![])
    }
    ...
    async fn stream_liquidations(&self, _symbol: &Symbol) -> Result<BoxStream<Liquidation>> {
        // KuCoin Futures does not expose public liquidation stream.
