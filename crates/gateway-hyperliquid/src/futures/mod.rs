...
pub struct HyperliquidFutures {
    config: ExchangeConfig,
    rest: rest::HyperliquidFuturesRest,
}

impl HyperliquidFutures {
    pub fn new(config: ExchangeConfig) -> Self {
        let rest = rest::HyperliquidFuturesRest::new(&config);
        Self { config, rest }
    }
...
#[async_trait]
impl Exchange for HyperliquidFutures {
...
#[async_trait]
impl FuturesExchange for HyperliquidFutures {
    ...
    async fn liquidations(&self, _symbol: &Symbol, _limit: u16) -> Result<Vec<Liquidation>> {
        // Hyperliquid does not expose public liquidation data.
        Ok(vec![])
    }
