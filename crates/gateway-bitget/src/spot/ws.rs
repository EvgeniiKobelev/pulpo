...
/// Connect to the Bitget V2 public spot WebSocket, subscribe to the given
    ...
    tokio::spawn(async move {
        ...
        tokio::spawn(async move {
            ...
            loop {
                tokio::select! {
                    _ = interval.tick() => {
                        // We can't write directly from here, so we just keep the
                        // channel alive. The actual ping is sent in the main loop.
                    }
...
/// Build a Bitget WS subscription arg object.
    ...
    Ok(Box::pin(
        futures::stream::unfold(raw_stream, |mut stream| async move {
            loop {
                ...
                let trades: Vec<BitgetWsTradeRaw> =
