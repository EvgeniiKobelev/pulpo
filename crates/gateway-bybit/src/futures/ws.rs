...
/// Connect to the Bybit V5 public linear WebSocket, subscribe to the given
...
async fn subscribe_and_stream(
    ...
    let sub = serde_json::json!({"op": "subscribe", "args": topics.clone()});
    write
...
pub async fn stream_orderbook(
    ...
    Ok(Box::pin(raw_stream.filter_map(|json| async move {
        ...
        let raw: BybitLinearWsOrderBook = serde_json::from_value(data.clone()).ok()?;
...
///
/// Bybit sends linear ticker updates which include `markPrice` and
/// `indexPrice` fields. We subscribe to `tickers.{SYMBOL}` and extract
/// the mark price data.
...
///
/// Subscribes to `liquidation.{SYMBOL}` topic on the Bybit V5 linear
/// public WebSocket.
