...
pub struct BybitFutures {
    ...
    rest: rest::BybitLinearRest,
}

impl BybitFutures {
    pub fn new(config: ExchangeConfig) -> Self {
        let rest = rest::BybitLinearRest::new(&config);
        Self { config, rest }
    }
...
impl FuturesExchange for BybitFutures {
    async fn funding_rate(&self, symbol: &Symbol) -> Result<FundingRate> {
        let raw = self.rest.funding_rate(symbol).await?;
    ...
    async fn open_interest(&self, symbol: &Symbol) -> Result<OpenInterest> {
        ...
        Ok(raw.into_open_interest(symbol.clone()))
    }

    async fn liquidations(&self, _symbol: &Symbol, _limit: u16) -> Result<Vec<Liquidation>> {
        // Bybit does not have a REST endpoint for historical liquidations.
        // Liquidations are only available via WebSocket (liquidation.{SYMBOL} topic).
        Ok(vec![])
    }
