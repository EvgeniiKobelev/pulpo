...
/// Bybit Spot канал `orderbook.1000` шлёт 1000 уровней на snapshot и дельты
/// при изменениях. Эмитим всю поддерживаемую глубину.
const TOP_LEVELS: usize = 1000;

/// Сколько update'ов копим в буфер пока ждём REST snapshot на re-sync.
const MAX_BUFFER: usize = 1024;

/// Bybit Spot ограничивает 10 args на subscribe-фрейм, поэтому шлём их
/// чанками. На одно соединение можно вешать сколько угодно subscribe'ов
/// до общего лимита соединения.
...
async fn subscribe_and_stream(
    ...
    tokio::spawn(async move {
        ...
        'outer: loop {
            // Bybit `orderbook.1000` отдаёт snapshot 1000 уровней, но дельты
            // приходят только для top-N (на практике меньше 1000). Дальние
            // уровни stale в pulpo'шной LocalOrderBook. Каждые 5 мин
            // ре-сабскрайбим — Bybit отвечает type="snapshot" с актуальной
            // книгой, pulpo через diff_against_prev обновляет stale.
            ...
            loop {
                tokio::select! {
                    ...
                    _ = resub_interval.tick() => {
                        debug!("Bybit spot WS: periodic resubscribe to refresh stale levels");
                        let mut all_ok = true;
                        for chunk in topics.chunks(SUBSCRIBE_CHUNK) {
                            let sub = serde_json::json!({"op":"subscribe","args":chunk});
...
/// Стрим консистентного стакана (top-1000) для множества символов через канал
/// `orderbook.1000.{SYMBOL}`. Поддерживает локальный стакан per-symbol по
/// схеме Bybit:
///
/// 1. После subscribe канал шлёт `type="snapshot"` (полные 1000 уровней) → reset.
/// 2. Дальнейшие `type="delta"` валидируются: `u == local.last_u + 1`.
/// 3. `u==1` в delta → депth/уровень канала сбросился, reset как на snapshot.
/// 4. На gap (u > last_u + 1) → REST snapshot через `/v5/market/orderbook`
///    с буферизацией update'ов до его прихода.
/// 5. На initial sync эмитим top-1000.
/// 6. Каждый update эмитится как `OrderBook` с пришедшими уровнями.
pub async fn stream_orderbooks_batch(
    ...
    let rest = Arc::new(BybitRest::new(config));
...
async fn maintain_shard(
    ...
    loop {
        tokio::select! {
            ...
            _ = out_tx.closed() => {
                debug!("Bybit Spot maintain shard: consumer dropped");
                return;
            }

            ev = raw.next() => {
                let Some(json) = ev else {
                    debug!("Bybit Spot maintain shard: ws stream ended");
                    return;
                };
                if let Err(e) = handle_ws_event(&json, &mut states, &out_tx, &rest, &snap_tx).await {
                    debug!(error = %e, "bybit spot ws event ignored");
                }
            }
...
async fn handle_ws_event(
    ...
    if msg_type == "snapshot" || u == 1 {
        ...
        info!(
            ...
            kind = msg_type,
            "Bybit Spot: local book initialized"
        );
        ...
        let ob = if prev_book.last_update_id > 0 {
            ...
            state
                .book
                .to_orderbook(ExchangeId::BybitSpot, symbol, event_time, TOP_LEVELS)
        };
    ...
    // Stale событие (TCP race с REST snapshot'ом и т.п.).
    if u <= state.last_u {
        return Ok(());
    }

    // Gap: u должен быть == last_u + 1.
    if u != state.last_u + 1 {
        ...
        warn!(
            ...
            "Bybit Spot: sequence gap, triggering re-sync"
        );
...
async fn handle_snapshot(
    ...
    info!(
        ...
        was_resync = prev_book.last_update_id > 0,
        "Bybit Spot: REST bootstrap applied"
    );
    ...
    let ob = if prev_book.last_update_id > 0 {
        ...
        state
            .book
            .to_orderbook(ExchangeId::BybitSpot, snap.symbol.clone(), ts, TOP_LEVELS)
    };
    let _ = out_tx.send(ob).await;
}
...
fn spawn_bootstrap(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            // Bybit spot REST max limit = 200.
            match rest.orderbook(&symbol, 200).await {
                Ok(ob) => {
                    ...
                    let Some(seq) = ob.sequence else {
                        warn!(symbol = %symbol, "Bybit Spot snapshot missing sequence, retrying");
                        tokio::time::sleep(backoff).await;
                        backoff = (backoff * 2).min(Duration::from_secs(30));
                        continue;
                    };
                ...
                Err(e) => {
                    warn!(
                        ...
                        "Bybit Spot REST snapshot failed"
                    );
                    tokio::time::sleep(backoff).await;
