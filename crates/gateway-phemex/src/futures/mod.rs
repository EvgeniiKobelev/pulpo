...
pub struct PhemexFutures {
    config: ExchangeConfig,
    rest: rest::PhemexFuturesRest,
}

impl PhemexFutures {
    pub fn new(config: ExchangeConfig) -> Self {
        let rest = rest::PhemexFuturesRest::new(&config);
        Self { config, rest }
    }
...
#[async_trait]
impl Exchange for PhemexFutures {
    fn id(&self) -> ExchangeId {
        ExchangeId::PhemexFutures
    }
...
#[async_trait]
impl FuturesExchange for PhemexFutures {
    ...
    async fn liquidations(&self, _symbol: &Symbol, _limit: u16) -> Result<Vec<Liquidation>> {
        // Phemex does not expose public liquidation data via REST.
        Ok(vec![])
    }
