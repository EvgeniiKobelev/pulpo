//! Subscribe to order book updates for ALL Phemex perpetual symbols.
//!
...
//!
//! Run: cargo run -p gateway-phemex --example stream_orderbook_all

use gateway_core::{Exchange, Symbol};
use gateway_phemex::PhemexFutures;
...
async fn main() -> anyhow::Result<()> {
    ...
    let futures = PhemexFutures::public();

    // 1. Fetch all available symbols.
    //    Phemex limits to 5 WS connections × 20 subs = 100 symbols max.
    //    The batch method auto-caps at this limit.
    let info = futures.exchange_info().await?;
    let all_symbols: Vec<Symbol> = info.symbols.into_iter().map(|s| s.symbol).collect();
    println!("Fetched {} total symbols from Phemex Futures", all_symbols.len());
    let symbols: Vec<Symbol> = all_symbols.into_iter().take(100).collect();
    ...
    //    Phemex allows up to 20 subs per connection and 5 connections per client.
    //    The batch method automatically shards across multiple connections.
