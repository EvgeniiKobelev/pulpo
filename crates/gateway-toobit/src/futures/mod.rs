...
pub struct ToobitFutures {
    config: ExchangeConfig,
    rest: rest::ToobitFuturesRest,
}

impl ToobitFutures {
    pub fn new(config: ExchangeConfig) -> Self {
        let rest = rest::ToobitFuturesRest::new(&config);
        Self { config, rest }
    }
...
#[async_trait]
impl Exchange for ToobitFutures {
    fn id(&self) -> ExchangeId {
        ExchangeId::ToobitFutures
    }
...
#[async_trait]
impl FuturesExchange for ToobitFutures {
    ...
    async fn liquidations(&self, _symbol: &Symbol, _limit: u16) -> Result<Vec<Liquidation>> {
        // Toobit does not expose public liquidation data via REST.
        Ok(vec![])
    }
    ...
    async fn stream_liquidations(&self, _symbol: &Symbol) -> Result<BoxStream<Liquidation>> {
        // Toobit does not expose public liquidation stream.
