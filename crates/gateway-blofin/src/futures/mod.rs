...
pub struct BlofinFutures {
    config: ExchangeConfig,
    rest: rest::BlofinFuturesRest,
}

impl BlofinFutures {
    pub fn new(config: ExchangeConfig) -> Self {
        let rest = rest::BlofinFuturesRest::new(&config);
        Self { config, rest }
    }
...
#[async_trait]
impl Exchange for BlofinFutures {
    fn id(&self) -> ExchangeId {
        ExchangeId::BlofinFutures
    }
...
#[async_trait]
impl FuturesExchange for BlofinFutures {
    ...
    async fn liquidations(&self, _symbol: &Symbol, _limit: u16) -> Result<Vec<Liquidation>> {
        // BloFin does not expose public liquidation data via REST.
        Ok(vec![])
    }
    ...
    async fn stream_liquidations(&self, _symbol: &Symbol) -> Result<BoxStream<Liquidation>> {
        // BloFin does not expose public liquidation stream.
