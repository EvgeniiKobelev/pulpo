//! Basic REST API example for OKX.
//!
//! Demonstrates the unified Exchange trait with OKX spot market data.
//!
//! Run: cargo run -p gateway-okx --example basic_rest
...
async fn main() -> anyhow::Result<()> {
    ...
    // -- Exchange Info --
    let info = okx.exchange_info().await?;
    println!("=== OKX Exchange Info ===");
    println!("Trading pairs: {}\n", info.symbols.len());

    // -- Ticker --
    let tick = okx.ticker(&btc).await?;
    ...
    // -- Order Book --
    let ob = okx.orderbook(&btc, 5).await?;
    ...
    // -- Recent Trades --
    let trades = okx.trades(&btc, 5).await?;
    ...
    // -- Candles --
    let candles = okx.candles(&btc, Interval::H1, 3).await?;
