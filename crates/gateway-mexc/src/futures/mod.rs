...
pub struct MexcFutures {
    ...
    rest: rest::MexcFuturesRest,
}

impl MexcFutures {
    pub fn new(config: ExchangeConfig) -> Self {
        let rest = rest::MexcFuturesRest::new(&config);
        Self { config, rest }
    }
...
#[async_trait]
impl FuturesExchange for MexcFutures {
    ...
    async fn mark_price(&self, symbol: &Symbol) -> Result<MarkPrice> {
        self.rest.fair_price(symbol).await
    }
    ...
    async fn liquidations(&self, _symbol: &Symbol, _limit: u16) -> Result<Vec<Liquidation>> {
        // MEXC does not provide a public API for liquidation data.
        Ok(Vec::new())
    }
    ...
    async fn stream_liquidations(&self, _symbol: &Symbol) -> Result<BoxStream<Liquidation>> {
        // MEXC does not provide liquidation stream data.
